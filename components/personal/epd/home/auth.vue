<template>
  <v-form ref="form" @submit="clickАuthorization" autocomplete="false">
    <div class="form_login_auth">
      <h4 >Вход в обьединеные платежи</h4>
    ...
    <div class="form_auth_personal" v-if="$store.state.user.user">
      <h4 >Здравствуйте! {{$store.state.user.last_name}}  {{$store.state.user.first_name}}</h4>
      <p>Ваш профиль:</p>
      <nuxt-link tag="div" to="/personal" class="user" >
        ...
        <div class="image" v-else>
          <img :src="'http://45.67.57.180' + $store.state.user.user.user_icon"  alt="">
        </div>
        <div class="user_info">
          <p v-if="$store.state.user.user.last_name != ''">{{$store.state.user.user.last_name}}  {{$store.state.auth.user.first_name}}</p>
          <p v-else>{{$store.state.user.user.email}}</p>
          <span>{{$store.state.user.user.organization}}</span>
        </div>
      </nuxt-link>
