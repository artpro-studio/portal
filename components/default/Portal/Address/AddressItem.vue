    ...
    <div v-for="(item, index) in propData" :key="index" class="item" @click="clickAddress(item.guid)">
      ...
      <div class="item__info">
        ...
        <div class="item__description">
          <p v-if="item.managementOrganization">Управляющая компания:  {{item.managementOrganization.shortName}}</p>
          <p v-if="item.postalCode">Индекс:  {{item.postalCode}}</p>
        </div>
      </div>
    </div>
  </div>
